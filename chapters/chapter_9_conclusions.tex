...
\begin{rqabox}
    \textbf{\Cref{itm:rq0}:}
    \emph{are \gls{SKI} and \gls{SKE} relevant in the context of modern \gls{AI}?}

    \textbf{Answer:}
    %
    not only \gls{SKI} and \gls{SKE} are relevant, but they are arguably one of the leading areas of research in \gls{AI} today, especially considering the recent advancements and integration with \gls{LLM} technologies.
    %
    In the near future, we expect many contributions in this area, both from a theoretical and practical standpoint, further solidifying the importance of \gls{SKI} and \gls{SKE} in the broader \gls{AI} landscape.
    %
\end{rqabox}
...
\begin{rqabox}
    \textbf{\Cref{itm:rq1}:}
    \emph{what are the characteristics of \gls{SKI} and \gls{SKE} techniques?}
    ...
    We believe that future contributions in the field of \gls{SKI} and \gls{SKE} will further diversify the characteristics of the techniques.
    %
    In particular, we expect more methods to leverage unstructured knowledge sources (e.g., text documents) for \gls{SKI}, and more expressive knowledge representations (e.g., ontologies with rich semantics, natural language) for \gls{SKE}.
\end{rqabox}
...
\begin{rqabox}
    \textbf{\Cref{itm:rq2}:}
    \emph{how can the effects of \gls{SKI} and \gls{SKE} be measured?}

    \textbf{Answer:}
    classical \gls{ML} metrics are still relevant to measure the performance of \gls{SKI} and \gls{SKE} techniques, but they are not sufficient to capture all the relevant aspects involved.
    %
    They should be complemented with additional metrics that capture other important aspects such as resource efficiency, robustness, and fairness depending on the context of application.
    %
    We acknowledge that the research on this topic is still in its early stages, and we believe that future researches will further refine and expand the set of metrics available to evaluate \gls{SKI} and \gls{SKE} techniques.
\end{rqabox}
...
\begin{rqabox}
    \textbf{\Cref{itm:rq3}:}
    \emph{when and where should \gls{SKI} and \gls{SKE} be used?}

    \textbf{Answer:}
    %
    \gls{SKI} techniques are particularly beneficial in scenarios where well established domain knowledge exists and can be leveraged to enhance the performance or other qualities of sub-symbolic models, including fairness and robustness.
    %
    On the other hand, \gls{SKE} techniques should be employed when there is a need to extract human-understandable knowledge, especially in domains where explainability and transparency are critical, such as healthcare.
\end{rqabox}
...
\begin{rqabox}
    \textbf{\Cref{itm:rq4}:}
    \emph{how to design and develop \gls{NeSy} \gls{AI} systems that leverage \gls{SKI} and \gls{SKE}?}

    \textbf{Answer:}
    \gls{SKI} and \gls{SKE} are fundamental components in the design and development of \gls{NeSy} \gls{AI} systems.
    %
    There are various ways to integrate these techniques, depending on the specific requirements and constraints of the application domain.
    %
    Generally speaking, these techniques should be employed whenever the \gls{AI} system deals with both symbolic and sub-symbolic knowledge representations or requirements (e.g., explainability, transparency, fairness).
    %
    For more advanced \gls{AI} systems, such as autonomous learning agents, \gls{SKI} and \gls{SKE} can be combined in many ways.
    %
    We suggest to use them in a cyclic manner to continuously acquire, integrate, and refine knowledge about the environment and the tasks at hand.
    %
    Also, the conjoint use of \glspl{LLM} and \gls{SKI}/\gls{SKE} techniques appears to be the promising direction for future research and applications.
    %
\end{rqabox}
