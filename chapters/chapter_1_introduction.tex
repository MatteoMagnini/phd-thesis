...
\Gls{ML} models, such as \glsplural{NN}, are able to learn from data and generalize to unseen data.
%
...
\begin{enumerate}[label=\emph{(\roman*)}]
    ...
    \begin{enumerate}[label=\emph{(\arabic*)},resume]
        %
        \item we systematically collect and organise into a taxonomy \gls{SKI} and \gls{SKE} methods and technologies (\Cref{itm:rq1});
        %
    ...
    \begin{enumerate}[label=\emph{(\arabic*)},resume]
        ...
        %
        \item we design and develop \gls{NeSy} \gls{AI} systems that leverage \gls{SKI} and \gls{SKE} techniques in real-world scenarios (\Cref{itm:rq3,itm:rq4});
        %
    \end{enumerate}
    %
    ...
    \begin{enumerate}[label=\emph{(\arabic*)},resume]
        %
        \item we investigate how \gls{SKI} techniques can be used to mitigate the risk of bias in \gls{AI} systems (\Cref{itm:rq3,itm:rq4});
        %
    \end{enumerate}
    %
\end{enumerate}
...
%
\Cref{ch:ai} introduces the reader to the field of \gls{AI}, where we present \emph{symbolic} and \emph{sub-symbolic} \gls{AI}.
%
\Cref{ch:nesy-ai} follows with a presentation of \gls{NeSy} \gls{AI}, with particular focus to \gls{SKI} and \gls{SKE}.
%
Finally, \Cref{ch:llm} brings the attention to \glspl{LLM}, which are the latest -- and still ongoing -- breakthrough in \gls{AI}.
