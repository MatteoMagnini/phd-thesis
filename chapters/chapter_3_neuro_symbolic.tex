...
In the rest of this chapter, we focus on two sub-fields of \gls{NeSy} \gls{AI} -- namely \gls{SKI} and \gls{SKE} -- that can be partially mapped into some of the aforementioned types of integration.
%
...
%
This necessitates the use of formal languages, such as \gls{FOL} or \glspl{DT}, for knowledge representation, while avoiding free text or natural language.
...
\begin{itemize}
    ...
    %
    The sub-categories (most of them already introduced in \Cref{subsec:symbolic-ai}), ordered by decreasing expressiveness, include:
    %
    \begin{itemize}
        %
...
%
The predictors targeted by the surveyed \gls{SKI} methods (most of them already introduced in \Cref{subsec:sub-symbolic-ai}) include:
%
\begin{itemize}
...
\begin{figure}
    ...
    \caption[SKI workflow of structuring strategy]{
        ...
        %
        The resulting predictor can then either be trained on data as is, or combined with an uneducated predictor to create a new educated predictor that is then trained on data.
    }
...
%
The formulas are then parsed ($\Pi$) into a syntactic structure (e.g., \gls{AST}).
%
...
%
The result of the fuzzification process is an \emph{educated} predictor ($P'$) that mimics the symbolic knowledge $\phi$.
%
At this point, $P'$ can be used as is to make predictions, or it can be further trained ($\tau$) to improve its predictive performance ($P'''$).
%
Another option is to combine $P'$ with an \emph{uneducated} predictor ($P$) to create a new predictor $P''$ that embodies both the symbolic knowledge and the \emph{uneducated} predictor's capabilities.
%
After $\tau$, the predictor is now both \emph{educated} $P'''$ with knowledge and trained on data.
%
...
\begin{figure}
    ...
    \caption[Venn diagram categorising SKE methods]{
        %
        Venn diagram categorising \gls{SKE} methods w.r.t.\ the formalism (\emph{shape}) used to represent the extracted symbolic knowledge: rule lists (L), decision trees (DT), decision tables (TA), or knowledge graphs (KG).
        %
    }
...
%
From a syntactic perspective, the formalism used to represent the extracted symbolic knowledge -- also referred as \emph{shape} for ease of reference in the rest of the thesis -- is crucial for its interpretability.
%
...
The majority of \gls{SKI} algorithms rely on symbolic knowledge represented as \glspl{KG} or propositional logic (\Cref{fig:pie-ski-logic}).
%
