...
The proposed architecture utilizes Horn clauses (see \Cref{subsubsec:horn-logic}) to represent both user preferences and expert prescriptions.
%
...
A separate \gls{NN} was trained for each of the 12 synthetic users.
%
...
%
First, limiting the growth of the \gls{DT} reduces computational complexity.
%
...
\section[RAG on open LLM for medical chatbot]{Applying \glsentryshort{RAG} on open \glspl{LLM} for a medical chatbot supporting hypertensive patients}
\label{sec:applying-rag-on-open-llm-for-a-medical-chatbot-supporting-hypertensive-patients}

In this section, we report the contribution of the paper ``Applying Retrieval-Augmented Generation on Open LLMs for a Medical Chatbot Supporting Hypertensive Patients'', presented at the 3rd Workshop on Artificial Intelligence for Healthcare (HC@AIxIA 2024), co-located with the 23rd International Conference of the Italian Association for Artificial Intelligence (AIxIA 2024)~\cite{DBLP:conf/hc/AguzziMFSM24}.
%
The paper explores the use of \gls{RAG} with open-source large language models to develop a medical chatbot for supporting hypertensive patients.
%
By integrating external, domain-specific medical knowledge into the generation process, the proposed approach aims to improve factual accuracy and reduce hallucinations.
%
The study provides empirical evidence on the feasibility of RAG-based architectures for safe and reliable patient-oriented digital health applications.
...
\begin{figure}
    ...
    \caption[\Glsentryshort{RAG} architecture]{
        \Glsentryshort{RAG} architecture.
        %
...
\begin{figure}[h]
    ...
    \end{subfigure}
    \caption[Performance comparison of \glsentryshort{RAG}-based systems against plain \gls{LLM} systems]{
      Performance comparison of \glsentryshort{RAG}-based systems against plain \gls{LLM} systems.
      %
      (a) Answer Correctness, (b) Answer Relevancy.
      %
      The results demonstrate that \glsentryshort{RAG} significantly enhances both answer correctness and relevancy across all tested models.
    }
