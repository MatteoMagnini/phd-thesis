...
\section[\Gls{NeSy} \Gls{AI} for disease diagnosis and mnitoring]{\Gls{NeSy} \gls{AI} for supporting Chronic disease diagnosis and monitoring}
...
%
These platforms often leverage \gls{AI} data-driven models~\cite{placeholder1, placeholder2}.
%
...
%
First, the performance of \gls{AI} models is often suboptimal, primarily due to insufficient dataset sizes for effectively training \gls{ML} algorithms~\cite{placeholder3}.
%
Second, even high-performing \gls{AI} models frequently operate as black boxes, making their outputs difficult to interpret.
%
This lack of transparency hinders adoption in clinical settings, where healthcare professionals require interpretable outputs to make informed decisions~\cite{placeholder4}.
%
...
Combining data-driven and knowledge-based models has been identified as a promising solution to enhance \gls{ML} performance and improve explainability~\cite{placeholder5}.
%
This work focuses on neuro-symbolic \gls{AI}, specifically \gls{SKI}, which injects symbolic knowledge into sub-symbolic \gls{ML} models~\cite{placeholder7}.
%
Among the various \gls{SKI} methods, we evaluate the \gls{LTN} approach~\cite{placeholder8}.
%
The potential of this approach is demonstrated using the \gls{PID} dataset for diabetes prediction~\cite{placeholder9}.
%
...
The integration of medical knowledge into \gls{ML} pipelines can occur at various stages~\cite{placeholder5, placeholder11}.
%
\begin{itemize}
...
\end{itemize}
%
While these strategies are promising, they often do not reference the \gls{SKI} framework or its complementary \gls{SKE} paradigm~\cite{placeholder7}.
%
These paradigms have proven effective in other domains and hold potential for healthcare applications.
%
Preliminary experiments~\cite{placeholder12} suggest that further evaluation of \gls{SKI} methods, such as \gls{LTN}, is warranted.


\subsection{Materials and Methods}
\label{subsec:materials-and-methods}
%
In this study, we utilize the \gls{PID} dataset~\cite{placeholder1}, a widely recognized benchmark in medical-related \gls{ML} research~\cite{placeholder2, placeholder3}.
%
We focus on the \gls{LTN} approach~\cite{placeholder4}, a \gls{SKI} method that combines structuring and constraining injection strategies.
%
...
%
The \gls{LTN} framework~\cite{placeholder4} extends \gls{FOL} with fuzzy semantics, where formulas are interpreted as continuous truth values in the interval \([0, 1]\).
%
...
%
Inspired by fidelity metrics in the \gls{SKE} framework~\cite{placeholder5}, we propose a metric to assess adherence to clinical rules.
%
...
%
Robustness is quantified using a reproducible procedure where training data is perturbed, and the model's performance is evaluated on the perturbed data~\cite{placeholder6}.
%
...
%
\begin{itemize}
    \item An input layer with 8 neurons, one for each feature in the dataset, using the Leaky Rectified Linear Unit (LeakyReLU) activation function and batch normalization.
    %
    \item Three hidden layers with 256, 128, and 64 neurons, respectively, each using LeakyReLU and batch normalization.
    %
...
%
The perturbations include noise addition, sample drop, and label flipping, following the methodology in~\cite{placeholder}.
%
